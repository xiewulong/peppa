...
* 测试环境(Testing) -> session存测试mongo数据库、图片存测试cdn、连接测试数据库
* 预生产环境(Pre-production) -> session存预生产mongo数据库、图片存预生产cdn、连接预生产数据库
* 生产环境(Production) -> session存生产mongo数据库、图片存生产cdn、连接生产数据库

根据开发-上线流程的不同 可能还会有更多的环境配置部分 如果都使用if...else...或switch来维护 可配置性和扩展性比较差 特别对当前环境来说其它环境的判断会显得很冗余 未免显得不够优雅
